...
class Game:
        ...
        for suit in suits:
            for value in range(7, 15):
                ...
                if key not in self._card_cache:
                    self._card_cache[key] = Card(key, value, suit)
    ...
    def play_card(self, player_index: int, card_index: int) -> bool:
        ...
        if top and not (card.suit == top.suit or card.value == top.value or card.value == 12):
            logger.error(f"Cannot play {card}: must match suit {top.suit} or value {top.value}")
            return False
    ...
    def _refresh_draw_pile(self) -> None:
        ...
        cards = list(self.discard_pile)[:-1]
    ...
    def check_game_over(self) -> bool:
        return sum(1 for p in self.players if p) <= 1
    ...
    def serialize(self) -> dict:
        return {
            ...
            "discard_pile": [{"name": c.name, "value": c.value, "suit": c.suit} for c in self.discard_pile],
